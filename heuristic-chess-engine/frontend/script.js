 ...
 *  - chess.js  v0.10.3  (move validation, FEN, SAN — UMD browser build)
...
// ---------------------------------------------------------------------------
// Maps the <select id="bot-strength"> value to a human label and the movetime
// (ms) the engine is allowed.  Movetime is the *maximum* wall-clock budget;
// the engine may also terminate early due to its depth cap at lower levels.
//
//  Elo   depth  noise_cp  movetime (ms)
//  800     1      200        500   ← shallow + heavy noise = beginner feel
//  1200    2       80        800   ← moderate noise
//  1500    3       30       1500   ← light noise, limited depth
//  1800    5        0       2000   ← solid tactical play
//  2000   64        0       3000   ← full strength (depth-limited by time)
const STRENGTH_CONFIG = {
  '800':  { label: '\ud83d\udfe3 Beginner  (~800 Elo)',   movetime:  500 },
  '1200': { label: '\ud83d\udd35 Casual    (~1200 Elo)',  movetime:  800 },
  '1500': { label: '\ud83d\udfe1 Normal    (~1500 Elo)',  movetime: 1500 },
  '1800': { label: '\ud83d\udfe0 Advanced  (~1800 Elo)',  movetime: 2000 },
  '2000': { label: '\ud83d\udd34 Maximum   (~2000 Elo)',  movetime: 3000 },
};

/** Return the currently selected strength key (e.g. '1800'). */
function getStrength () { return $('#bot-strength').val() || '1800'; }
...
async function startNewGame (color) {
  ...
  // Show which strength level is active.
  const strengthKey    = getStrength();
  const strengthLabel  = (STRENGTH_CONFIG[strengthKey] || STRENGTH_CONFIG['1800']).label;
  setStatus(`Game started — ${strengthLabel}`, 'good');
  logMsg(`New game • You play ${capitalize(playerColor)} • Bot: ${strengthLabel}`, 'system');
  updateEngineInfo({ depth: '—', score_text: '—', pv: '—' });
  $('#info-strength-badge').text(strengthLabel);
  updateEvalBar(0);
...
async function requestEngineMove () {
  ...
  const strengthKey = getStrength();
  const cfg         = STRENGTH_CONFIG[strengthKey] || STRENGTH_CONFIG['1800'];
  const movetime    = cfg.movetime;   // ms; auto-derived from strength level
  const fen         = game.fen();
  ...
  try {
    const resp = await fetch('/api/move', {
      ...
      body:    JSON.stringify({ fen, movetime, strength: strengthKey }),
    });
