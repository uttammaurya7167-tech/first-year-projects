...
class Searcher:
    ...
    def iterative_deepening(self, board: chess.Board,
                            max_depth: int = 64,
                            time_limit: float = 0.0) -> chess.Move:
        """Search with iterative deepening until depth or time is exhausted.

        Iterative deepening works by running a complete Alpha-Beta search at
        depth 1, then depth 2, etc. Each shorter search provides:
          - A good first move (hash move) for the deeper search, improving
            move ordering significantly.
          - An "anytime" result: if time runs out mid-search we still have the
            best move from the previous completed iteration.
        ...
        for depth in range(1, max_depth + 1):
            # Run Alpha-Beta at this depth.
            score = self.alpha_beta(board, depth, -INFINITY, INFINITY, ply=0)

            if self.stop:
                # Time ran out mid-search; use the result from the last
                # completed iteration.
                break

            # Retrieve the best move this iteration found.
            key = chess.polyglot.zobrist_hash(board)
            tt_entry = self.tt.get(key)
            ...
            # Info string for UCI output (printed by main.py).
            yield depth, best_score, best_move, elapsed, self.nodes

            # Stop if forced mate is found.
